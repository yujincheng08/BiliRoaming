...
class CDNHook(classLoader: ClassLoader, private val processName: String) : BaseHook(classLoader) {
    override fun startHook() {
        ...
        if (processName.endsWith(":download")) {
            ...
            if (!sPrefs.getBoolean("use_cdn", false)) return
    ...
    private fun getCDN(): Pair<String, Int>? {
        if (!sPrefs.getBoolean("use_cdn", false)) return null
        var cdn = sPrefs.getString("cdn", "")
        if (cdn.isNullOrEmpty()) cdn = sPrefs.getString("custom_cdn", "") ?: return null
